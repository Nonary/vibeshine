...
namespace display_device::policy {
  ...
  bool effective_hdr_requested(const session_t &session) { return session.enable_hdr && !session.prefer_sdr_10bit && !session.force_sdr; }
  bool effective_10bit_sdr_requested(const session_t &session) {
    return session.prefer_sdr_10bit && session.enable_hdr && !session.force_sdr;
  }
