  ...
  <div class="space-y-6">
    ...
    <section v-if="platform === 'windows'" class="space-y-3">
      ...
      >
        ...
        <n-alert v-if="statusKind === 'not_running'" type="warning" :show-icon="true">
          {{ $t('playnite.playnite_not_running_desc') }}
        </n-alert>
        <div class="flex items-center gap-2">
          <n-button type="primary" size="small" :loading="installing" @click="showInstallDialog">
            <i class="fas fa-plug" />
            <span class="ml-2">{{
              status.installed
          ...
          <span v-if="installOk" class="text-success text-xs">{{
            installErrorMsg || $t('playnite.install_success')
          }}</span>
    ...
    <n-modal v-model:show="showInstallModal">
      <n-card
        style="width: 500px"
        :title="$t('playnite.install_modal_title')"
        :bordered="false"
        size="huge"
        role="dialog"
        aria-modal="true"
      >
        <template #header-extra>
          <n-button quaternary circle @click="cancelInstall">
            <i class="fas fa-times" />
          </n-button>
        </template>

        <div class="space-y-4">
          <p>{{ $t('playnite.install_modal_message') }}</p>
        </div>

        <template #footer>
          <div class="flex justify-end gap-2">
            <n-button @click="cancelInstall">
              {{ $t('playnite.install_modal_cancel') }}
            </n-button>
            <n-button type="primary" :loading="installing" @click="performInstall(true)">
              {{ $t('playnite.install_modal_confirm') }}
            </n-button>
          </div>
        </template>
      </n-card>
...
async function refreshStatus() {
  ...
  try {
    ...
    if (r.status === 200 && r.data) {
      ...
      status.playnite_running = !!r.data.playnite_running;
      status.extensions_dir = r.data.extensions_dir || '';
    }
  } catch (error) {
    console.warn('Failed to refresh Playnite status:', error);
  }
}
...
async function showInstallDialog() {
  // If Playnite is running and the plugin is being installed/reinstalled, show modal
  if (status.playnite_running && status.installed) {
    showInstallModal.value = true;
  } else {
    // For first time installations or when Playnite is not running, install directly
    await performInstall(status.playnite_running);
  }
}

async function performInstall(restartPlaynite: boolean = false) {
  ...
  try {
    const requestBody = restartPlaynite ? { restart_playnite: true } : {};
    const r = await http.post('/api/playnite/install', requestBody, { validateStatus: () => true });
    let ok = false;
    let body: any = null;
    try {
      body = r.data;
    ...
    if (ok) {
      ...
      if (body.restarted) {
        installErrorMsg.value = t('playnite.restart_success');
      } else if (body.restart_warning) {
        installErrorMsg.value = t('playnite.restart_warning') + ': ' + body.restart_warning;
      }

      // Refresh status after a short delay to allow Playnite to start
      setTimeout(async () => {
        await refreshStatus();
      }, 3000);
    } else {
...
// Legacy function for backward compatibility - now handled by showInstallDialog
...
const statusKind = computed<'active' | 'session' | 'not_running' | 'uninstalled'>(() => {
  ...
  if (!status.playnite_running) return 'not_running';
  return 'active';
});
...
const statusText = computed<string>(() => {
  switch (statusKind.value) {
    ...
    case 'not_running':
      return t('playnite.status_playnite_not_running');
    case 'session':
