# Vibeshine 1.18.3-stable.4 - 2026-08-05
...
This stable release includes substantial changes to Windows capture, encoding, virtual displays, browser streaming, and the Web UI. If you find a bug or regression, please report it at https://github.com/Nonary/vibeshine/issues and include your logs, GPU, client, and display details when possible.

## Changes
- Added virtual-display capability and driver-readiness status to client discovery so compatible clients can offer a recoverable virtual-display launch.
...
## Fixes
- Fixed stale saved display identities preventing otherwise available encoders from being discovered. Capability probing now selects a live output on the requested GPU, and the v2 overview reports failed probe attempts for log review.
- Prevented temporary virtual displays and unavailable topology identities from disturbing the physical desktop during RTSP or browser streaming. Display configuration now retries safely when Windows cannot enumerate displays and otherwise preserves the established fallback to a usable capture output.
...
- Improved virtual-display creation, HDR verification, recovery, and teardown; updated libvirtualdisplay to v1.6.2 to keep newly created headless displays alive and kept RTSP startup within Moonlight's initial-video deadline.
