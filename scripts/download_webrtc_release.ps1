...
function New-GitHubHeaders {
  ...
  $headers = @{
    ...
    "User-Agent" = "vibeshine-webrtc-downloader"
...
if (-not $OutDir) {
  $OutDir = if ($env:WEBRTC_ROOT) { $env:WEBRTC_ROOT } else { Join-Path $scriptRoot ".vibeshine-deps\libwebrtc\out" }
}
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) "vibeshine-webrtc-$([System.Guid]::NewGuid().ToString('N'))"
