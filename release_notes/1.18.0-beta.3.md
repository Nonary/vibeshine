# Vibeshine 1.18.0-beta.3 - 2026-06-28
...
This release fixes application saving, which could silently fail in recent 1.18.0 prereleases, and makes the Vibeshine virtual display the default capture path for fresh installs.

## Changes
- Fresh installs now default to the Vibeshine Display Driver with per-client exclusive virtual displays, while SudoVDA remains available as an explicit rollback option. Existing config keys are preserved, with the visible control inverted to "Use SudoVDA", and all locales describe the new default/rollback behavior.
