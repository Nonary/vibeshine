...
static Game G(std::string id, std::string last, bool installed=true, std::vector<std::string> cats={}) {
  Game g; g.id=id; g.name=id; g.last_played=last; g.installed=installed; g.categories=cats; return g;
}
...
TEST(PlayniteSync_Recent, SortsByLastPlayedAndRespectsLimit) {
  ...
  std::unordered_set<std::string> excl; std::unordered_map<std::string,int> flags;
...
TEST(PlayniteSync_Indexes, MatchAppByIdThenCmdThenDir) {
  ...
  sel[1].exe = "C:/Games/Game.exe"; sel[1].working_dir = "C:/Games";
  std::unordered_map<std::string, GameRef> by_exe, by_dir, by_id; build_game_indexes(sel, by_exe, by_dir, by_id);

  nlohmann::json app;
  // Prefer id
  app["playnite-id"] = "ID2";
  auto g = match_app_against_indexes(app, by_id, by_exe, by_dir);
  ASSERT_NE(g, nullptr); EXPECT_EQ(g->id, "ID2");

  // No id, match by cmd
  nlohmann::json app2; app2["cmd"] = "\"C:/Games/Game.exe\""; // quotes and forward slashes acceptable
  g = match_app_against_indexes(app2, by_id, by_exe, by_dir);
  ASSERT_NE(g, nullptr); EXPECT_EQ(g->id, "ID2");

  // No id/cmd, match by working-dir
  nlohmann::json app3; app3["working-dir"] = "C:/Games";
...
TEST(PlayniteSync_TTL, NoDeleteWhenDisabledOrPlayedAfterAdded) {
  nlohmann::json app; app["playnite-id"] = "X"; app["playnite-added-at"] = "2024-01-01T00:00:00Z";
...
TEST(PlayniteSync_Purge, RemovesUninstalledAndOptionallyNonSelectedWhenReplacementAvailable) {
  nlohmann::json root; root["apps"]=nlohmann::json::array();
  // Two auto apps: A and B
  for (auto id: {"A","B"}) { nlohmann::json a; a["playnite-id"]=id; a["playnite-managed"]="auto"; root["apps"].push_back(a); }
  std::unordered_set<std::string> uninstalled_lower{ to_lower_copy(std::string("B")) };
  auto now=std::time(nullptr); std::unordered_map<std::string,std::time_t> last_played;
  ...
  nlohmann::json root2; root2["apps"]=nlohmann::json::array(); nlohmann::json x; x["playnite-id"]="X"; x["playnite-managed"]="auto"; root2["apps"].push_back(x);
  ...
  nlohmann::json root3; root3["apps"]=nlohmann::json::array(); nlohmann::json x2; x2["playnite-id"]="X"; x2["playnite-managed"]="auto"; root3["apps"].push_back(x2);
  changed=false;
...
TEST(PlayniteSync_AddMissing, AddsMissingSelectedWithMetadataAndTimestamps) {
  ...
  std::unordered_map<std::string,int> src_flags; src_flags["S1"]=1; src_flags["S2"]=3;
  bool changed=false;
...
TEST(PlayniteSync_CurrentAutoIds, CollectsOnlyAutoManaged) {
  nlohmann::json root; root["apps"]=nlohmann::json::array();
  nlohmann::json a; a["playnite-id"]="A"; a["playnite-managed"]="auto"; root["apps"].push_back(a);
  nlohmann::json b; b["playnite-id"]="B"; root["apps"].push_back(b);
