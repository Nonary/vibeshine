...
namespace platf::virtual_display_cleanup {
  // Invalidate any bounded retry retained by an older teardown. Stream starts
  // call this before waiting for the display transaction so stale cleanup
  // cannot acquire immediately afterward and remove the new display.
  ...
  // Restore the helper/database baseline without removing retained virtual
  // displays. Used when an aborted start may have modeset an existing target.
