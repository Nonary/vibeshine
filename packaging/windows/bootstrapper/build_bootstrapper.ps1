...
param(
    ...
    [string]$SignPathProjectSlug = $(if ([string]::IsNullOrWhiteSpace($env:SIGNPATH_PROJECT_SLUG)) { "Vibeshine" } else { $env:SIGNPATH_PROJECT_SLUG }),
    [string]$SignPathSigningPolicySlug = $(if ([string]::IsNullOrWhiteSpace($env:SIGNPATH_SIGNING_POLICY_SLUG)) { "test-sign" } else { $env:SIGNPATH_SIGNING_POLICY_SLUG }),
