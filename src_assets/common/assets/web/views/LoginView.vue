<template>
  <div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-solar-light dark:bg-lunar-dark text-solar-dark dark:text-lunar-light">
    <div class="w-full max-w-md space-y-8 rounded-lg p-8 shadow-sm bg-solar-surface dark:bg-lunar-surface border border-solar-secondary/10 dark:border-lunar-secondary/10">
      <div class="flex flex-col items-center mb-4">
        <img src="/images/logo-sunshine-45.png" height="45" alt="Sunshine" class="mb-2">
        <h1 class="text-xl font-semibold text-solar-secondary dark:text-lunar-onLight">{{ t('auth.login_title') }}</h1>
      </div>

  <form @submit.prevent="login" v-if="!isLoggedIn" autocomplete="on" class="space-y-4">
        <div>
          <label for="username" class="block text-sm font-medium text-solar-secondary/80 dark:text-lunar-onLight/80 mb-1">{{ t('_common.username') }}</label>
          <input
            type="text"
            id="username"
            name="username"
        ...
        <div>
          <label for="password" class="block text-sm font-medium text-solar-secondary/80 dark:text-lunar-onLight/80 mb-1">{{ t('_common.password') }}</label>
          <input
            type="password"
            id="password"
            name="password"
        ...
        <div v-if="error" class="mt-3 text-sm rounded p-2 text-solar-danger bg-solar-danger/10 border border-solar-danger/20 dark:text-lunar-danger dark:bg-lunar-danger/10 dark:border-lunar-danger/20">
          {{ error }}
        </div>
      </form>

      <div v-else class="text-center">
        <div class="mt-4 inline-flex items-center px-3 py-2 rounded text-solar-success bg-solar-success/10 border border-solar-success/20 dark:text-lunar-success dark:bg-lunar-success/10 dark:border-lunar-success/20">
