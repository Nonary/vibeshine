<template>
  <div class="container mx-auto max-w-4xl">
    ...
    <!-- Pair New Client -->
    <div class="card p-4 mb-6 shadow-sm border border-solar-dark/10 dark:border-lunar-light/10 rounded-md bg-white dark:bg-lunar-surface">
      ...
      <form @submit.prevent="registerDevice" class="flex flex-col gap-3 md:flex-row md:items-end">
        <div class="flex flex-col flex-1">
          <label class="text-xs font-semibold uppercase tracking-wide mb-1" for="pin-input">{{ $t('navbar.pin') }}</label>
          <input
            id="pin-input"
            v-model="pin"
            type="text"
        ...
        <div class="flex flex-col flex-1">
          <label class="text-xs font-semibold uppercase tracking-wide mb-1" for="name-input">{{ $t('pin.device_name') }}</label>
          <input
            id="name-input"
            v-model="deviceName"
            type="text"
    ...
    <!-- Existing Clients -->
    <div class="card p-4 shadow-sm border border-solar-dark/10 dark:border-lunar-light/10 rounded-md bg-white dark:bg-lunar-surface mb-6">
      ...
      <div v-if="unpairAllStatus === true" class="alert alert-success mb-3">{{ $t('troubleshooting.unpair_all_success') }}</div>
      <div v-if="unpairAllStatus === false" class="alert alert-danger mb-3">{{ $t('troubleshooting.unpair_all_error') }}</div>
      <ul
      ...
      <div
        v-else
        class="p-4 text-center italic opacity-75"
      >
...
.btn { display: inline-flex; align-items: center; gap: 0.5rem; border: none; border-radius: 0.375rem; font-weight: 500; cursor: pointer; }
.btn-primary { background: #ffc400; color: #fff; padding: 0.5rem 1rem; }
.btn-primary:disabled { opacity: .5; cursor: not-allowed; }
.btn-primary:not(:disabled):hover { filter: brightness(0.95); }
.btn-danger { background: #dc2626; color: #fff; padding: 0.5rem 0.75rem; }
.btn-danger:disabled { opacity: .5; cursor: not-allowed; }
.btn-danger:not(:disabled):hover { background: #b91c1c; }
.btn-sm { font-size: .875rem; padding: 0.25rem 0.5rem; }
.alert { padding: 0.5rem 0.75rem; border-radius: 0.375rem; font-size: .875rem; }
