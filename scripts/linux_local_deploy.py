...
STATE = Path('/var/lib/vibeshine-local-deploy')
TRUSTED_PATH = '/usr/sbin:/usr/bin:/sbin:/bin'
HOST = 'vibeshine.service'
...
UNITS = (HOST, CONTROLLER, SOCKET, 'vibeshine-session-exec@.service',
...
FIXED = {
    ...
    'usr/lib/firewalld/services/vibeshine.xml', 'etc/ufw/applications.d/vibeshine',
    'usr/share/pipewire/pipewire.conf.d/50-vibeshine-audio.conf',
    'usr/share/metainfo/io.github.Nonary.vibeshine.metainfo.xml',
}
FIXED.update(f'usr/libexec/vibeshine/vibeshine-{name}' for name in HELPERS)
FIXED.update(f'usr/lib/systemd/system/{name}' for name in UNITS)
FIXED.update(f'usr/share/applications/io.github.Nonary.vibeshine{suffix}.desktop'
             for suffix in ('', '.kwin', '.terminal'))
FIXED.update(f'usr/share/icons/hicolor/scalable/{kind}/io.github.Nonary.vibeshine{suffix}.svg'
             for kind, suffix in [('apps', ''), ('status', '-locked'), ('status', '-pausing'),
                                  ('status', '-playing'), ('status', '-tray')])
FIXED.update(f'usr/lib/systemd/user/{unit}.service.d/{dropin}.conf'
             for unit in ('plasma-kwin_wayland', 'plasma-login-kwin_wayland')
             for dropin in ('vibeshine-kwin-gpu', 'vibeshine-kwin-session-environment'))
VERSION = re.compile(r'[1-9][0-9]*\.[0-9]+\.[0-9]+(?:-(?:alpha|beta|rc|stable)\.[0-9]+)?')
TRANSACTION_ID = re.compile(r'[0-9]{8}T[0-9]{6}Z-[0-9a-f]{8}')
CAPABILITIES = {
    'usr/libexec/vibeshine/vibeshine-host': 'cap_sys_admin,cap_sys_nice=p',
    'usr/libexec/vibeshine/vibeshine-session-broker': 'cap_kill,cap_setgid,cap_setuid=p',
}
...
def inspect_archive(archive, version):
    ...
    for entry in archive:
        ...
        if not entry.isdir():
            ...
            if entry.issym() and (name != 'usr/bin/vibeshine' or
    ...
    if not members['usr/bin/vibeshine'].issym():
        raise DeployError('Public executable must point to the versioned candidate')
    if {name for name in members if name.startswith('usr/bin/vibeshine-') and
            VERSION.fullmatch(name.removeprefix('usr/bin/vibeshine-'))} != {f'usr/bin/vibeshine-{version}'}:
...
class Files:
    ...
    def replace(self, name, source=None, link=None, mode=0o644, uid=0, gid=0, xattrs=None):
        ...
        fd, temporary_name = tempfile.mkstemp(prefix='.vibeshine-install-', dir=target.parent)
...
def broker_units():
    ...
    if any(not re.fullmatch(r'vibeshine-session-exec@[A-Za-z0-9_.\\-]+\.service', name) for name in names):
...
def quiesce():
    ...
    run('/usr/libexec/vibeshine/vibeshine-session-controller', 'cleanup', timeout=45)
    ...
    if Path('/run/vibeshine/session.env').exists():
...
def power_probe():
    record = Path('/run/vibeshine/session.env')
    ...
    process = subprocess.Popen([
        ...
        f'VIBESHINE_SESSION_GENERATION={generations[0]}',
        '/usr/libexec/vibeshine/vibeshine-session-exec', 'display-power',
...
def readiness(timeout):
    ...
    if (active_session.returncode == 0 and (not session or not graphical) and
            not Path('/run/vibeshine/session.env').exists() and
...
def install_mode(name, _source_mode=0):
    if name.endswith('/vibeshine-host'):
        return 0o750
    if name.endswith('/vibeshine-session-broker'):
        return 0o700
...
def install_driver(directory, manifest):
    ...
    for kernel in manifest['driver']['kernels']:
        print(f'Building/installing Vibeshine DRM for {kernel}', flush=True)
        code = driver_command(['/usr/bin/bash', '-c', wrapper, 'vibeshine-driver-upgrade', kernel])
...
def root_install(args):
    ...
    if digest(candidate / f'usr/bin/vibeshine-{args.version}') != digest(candidate / 'usr/libexec/vibeshine/vibeshine-host'):
    ...
    if os.path.lexists('/run/systemd/system/vibeshine-session-exec.socket'):
    ...
    for tree in ('usr/share/vibeshine', 'usr/lib/vibeshine'):
    ...
    for path in Path('/usr/bin').glob('vibeshine-*'):
        if VERSION.fullmatch(path.name.removeprefix('vibeshine-')):
            info = path.lstat()
            if not stat.S_ISREG(info.st_mode) or info.st_uid != 0 or info.st_mode & 0o022:
    ...
    for name in names:
        ...
        if member is not None:
            manifest['intended_metadata'][name] = {
                'uid': 0, 'gid': account.pw_gid if name.endswith('/vibeshine-host') else 0,
        ...
        for name in sorted(names):
            ...
            if member is None:
                ...
                files.replace(name, candidate / name, link=member.linkname if member.issym() else None,
                              ...
                              gid=account.pw_gid if name.endswith('/vibeshine-host') else 0)
        for path, caps in (
            ('/usr/libexec/vibeshine/vibeshine-host', 'cap_sys_admin,cap_sys_nice+p'),
            ('/usr/libexec/vibeshine/vibeshine-session-broker', 'cap_setgid,cap_setuid,cap_kill+p'),
        ):
...
def verify_payload(directory, manifest, members):
    account = pwd.getpwnam('vibeshine')
    if os.path.samefile('/usr/bin/vibeshine', '/usr/libexec/vibeshine/vibeshine-host'):
    ...
    for name, expected in manifest['after'].items():
        ...
        if expected is not None and 'link' not in expected:
            info = target.lstat()
            wanted_gid = account.pw_gid if name.endswith('/vibeshine-host') else 0
...
def platform_preflight(native=True):
    ...
    if not kernel or tuple(map(int, kernel.groups())) < (6, 16):
        raise DeployError('Managed Vibeshine displays require Linux 6.16 or newer')
...
def configure_command(args, build, cache):
    ...
    command = ['cmake', '-S', str(REPO), '-B', str(build), '-G', 'Ninja',
               ...
               '-DSUNSHINE_ASSETS_DIR=/usr/share/vibeshine', '-DSUNSHINE_EXECUTABLE_PATH=/usr/bin/vibeshine',
...
def build_install(args):
    ...
    with (build / '.local-deploy.lock').open('w') as lock, tempfile.TemporaryDirectory(prefix='vibeshine-local-stage-') as temporary:
        ...
        reported = run(work / 'stage' / f'usr/bin/vibeshine-{args.version}', '--version',
                       env=version_environment).stdout
        if not re.search(r'Vibeshine version: ' + re.escape(args.version) + r'(?:\s|$)', reported):
