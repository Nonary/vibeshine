...
Write-Output "Sunshine root directory: $rootDir"
...
try {
    ...
    if ($Action -eq "add") {
        ...
        if ($changed) {
            ...
            Write-Output "Successfully added Sunshine directories to PATH"
        } else {
    ...
    if ($Action -eq "remove") {
        ...
        if ($removedEntries.Count -gt 0) {
            ...
            Write-Output "Successfully removed Sunshine directories from PATH"
        } else {
            foreach ($pathToRemove in $pathsToManage) {
    ...
    if ($Action -eq "add") {
        Write-Output "Failed to add Sunshine directories to PATH"
    } else {
        Write-Output "Failed to remove Sunshine directories from PATH"
    }
