                ...
                <div
                    ...
                    <header class="px-6 py-5 border-b border-black/10 dark:border-white/10 flex items-center gap-3">
                        <div class="flex-1">
                            ...
                            <p class="mt-1 text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400"
                                v-if="!credentialsConfigured">
                                {{ t('auth.first_user_subtitle') }}
                            </p>
                        </div>
                    </header>
                    <form class="px-6 py-6 space-y-5" @submit.prevent="submit" novalidate>
                        <div class="space-y-1">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{
                                t('auth.username') }}</label>
                            <input v-model.trim="username" required autocomplete="username" autofocus
                                class="w-full rounded-lg border border-gray-300/70 dark:border-gray-600/70 bg-white/70 dark:bg-neutral-800/70 px-3 py-2 text-sm placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-solar-primary/50 dark:focus:ring-lunar-primary/50 focus:border-transparent transition" />
                        </div>
                        <div class="space-y-1" v-if="credentialsConfigured">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{
                                t('auth.password') }}</label>
                            <input v-model.trim="password" type="password" required autocomplete="current-password"
                                class="w-full rounded-lg border border-gray-300/70 dark:border-gray-600/70 bg-white/70 dark:bg-neutral-800/70 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-solar-primary/50 dark:focus:ring-lunar-primary/50 focus:border-transparent transition" />
                        </div>
                        <template v-else>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class="space-y-1 sm:col-span-2">
                                    ...
                                    <input v-model.trim="newPassword" type="password" required
                                ...
                                <div class="space-y-1 sm:col-span-2">
                                    ...
                                    <input v-model.trim="confirmNewPassword" type="password" required
                                        autocomplete="new-password"
                                        class="w-full rounded-lg border border-gray-300/70 dark:border-gray-600/70 bg-white/70 dark:bg-neutral-800/70 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-solar-primary/50 dark:focus:ring-lunar-primary/50 focus:border-transparent transition" />
                                </div>
                            </div>
                        ...
                        <div class="min-h-[1.25rem]">
                            <p v-if="error"
                                class="text-sm font-medium text-red-600 dark:text-red-400 flex items-center gap-2"><i
                                    class="fas fa-triangle-exclamation" /> {{ error }}</p>
                            <p v-else-if="success"
                        ...
                        <div class="flex items-center justify-end pt-1">
                            <button :disabled="submitting"
                                class="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold tracking-wide bg-solar-primary dark:bg-lunar-primary text-solar-onPrimary dark:text-lunar-onPrimary shadow disabled:opacity-50 disabled:cursor-not-allowed hover:brightness-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-solar-primary/60 dark:focus-visible:ring-lunar-primary/60 transition">
                                <span v-if="!credentialsConfigured">{{ submitting ? t('auth.creating_user') :
                                    t('auth.create_user') }}</span>
...
async function submit() {
    ...
    try {
        if (!credentialsConfigured.value) {
            ...
            const res = await http.post('/api/password', {
        ...
        const loginRes = await http.post('/api/auth/login', {
            ...
            password: credentialsConfigured.value ? password.value || newPassword.value : newPassword.value,
