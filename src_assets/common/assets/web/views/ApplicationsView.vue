<template>
	<div class="space-y-6">
		...
		<div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">
			<div v-for="(app,i) in apps" :key="appKey(app,i)" class="group relative rounded-xl overflow-hidden border border-black/10 dark:border-white/10 bg-white/60 dark:bg-lunar-surface/60 shadow hover:shadow-lg transition flex flex-col">
			 <div class="aspect-[3/4] w-full bg-black/10 dark:bg-white/5 relative">
			<img v-if="hasCover(app)"
				:src="coverSrc(app, i)"
				:key="appKey(app,i)"
				...
				<div v-else class="absolute inset-0 flex items-center justify-center text-4xl font-bold text-solar-primary/40 dark:text-lunar-primary/40">{{ app.name?.substring(0,1)||'?' }}</div>
					<div class="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition">
						<button class="mini-btn" @click="openEdit(app,i)"><i class="fas fa-cog"></i></button>
					</div>
				</div>
				<div class="p-3 flex flex-col flex-1 min-h-[110px]">
					<h3 class="text-sm font-semibold truncate">{{ app.name }}</h3>
					<p class="text-[10px] uppercase tracking-wide opacity-60 mb-2 line-clamp-2">{{ displayCmd(app) }}</p>
					<div class="mt-auto flex items-center justify-between text-[10px]">
						<span class="px-2 py-0.5 rounded-full bg-green-500/15 text-green-600 dark:text-green-400">Configured</span>
						<span v-if="app['prep-cmd']?.length" class="opacity-60">{{ app['prep-cmd'].length }} prep</span>
					</div>
				</div>
			</div>
			<button @click="openAdd" class="aspect-[3/4] rounded-xl border border-dashed border-black/20 dark:border-white/15 flex flex-col items-center justify-center text-black/40 dark:text-white/30 hover:text-solar-primary dark:hover:text-lunar-primary hover:border-solar-primary/50 dark:hover:border-lunar-primary/50 transition">
				<i class="fas fa-plus text-3xl mb-2"></i>
				<span class="text-xs font-medium">Add Application</span>
			</button>
		</div>
		<AppEditModal v-model="showModal" :platform="platform" :app="currentApp" :index="currentIndex" @saved="reload" @deleted="reload" />
	</div>
</template>
<script setup>
...
<style scoped>
.main-btn{ display:inline-flex; align-items:center; gap:6px; background:rgba(253,184,19,0.9); color:#212121; font-size:11px; font-weight:500; padding:6px 12px; border-radius:6px; }
...
.mini-btn{ background:rgba(0,0,0,.65); color:#fff; font-size:11px; padding:4px 8px; border-radius:6px; }
