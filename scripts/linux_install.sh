...
check_session_restart() {
  ...
  if ! pacman -Q vibeshine >/dev/null 2>&1; then
    reboot_required=1
    warn 'First installation requires a reboot so the login screen and desktop load the Vibeshine display and session integration.'
  fi
}
