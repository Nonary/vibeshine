...
<template>
  <div id="general" class="config-page">
    ...
    <div class="mb-6">
      ...
      <input id="sunshine_name" v-model="config.sunshine_name" type="text" class="form-control"
    ...
    <div id="global_prep_cmd" class="mb-6 flex flex-col">
      ...
      <div v-if="config.global_prep_cmd.length > 0" class="mt-3 space-y-3">
        <div v-for="(c, i) in config.global_prep_cmd" :key="i" class="grid grid-cols-12 gap-2 items-start">
          ...
          <div class="col-span-1 flex gap-2">
            <button class="inline-flex items-center px-3 py-1.5 rounded-md bg-danger text-onLight"
              @click="removeCmd(i)">
              <i class="fas fa-trash" />
            </button>
            <button class="inline-flex items-center px-3 py-1.5 rounded-md bg-success text-onLight" @click="addCmd">
              <i class="fas fa-plus" />
            </button>
          </div>
        </div>
      ...
      <div class="mt-4">
        <button class="mx-auto block inline-flex items-center px-3 py-1.5 rounded-md bg-success text-onLight"
          @click="addCmd">
          &plus; {{ $t('config.add') }}
        </button>
      </div>
    ...
    <!-- Session Token TTL -->
    <div class="mb-6">
      <label for="session_token_ttl_seconds" class="block text-sm font-medium mb-1 text-dark dark:text-light">{{
