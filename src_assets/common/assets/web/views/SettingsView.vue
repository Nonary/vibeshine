  ...
  <main ref="mainEl" class="flex-1 px-0 md:px-2 xl:px-6 py-2 md:py-6 space-y-6 overflow-x-hidden">
    <header
      ...
      <div class="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div v-if="!autoSave" class="flex gap-2">
          ...
          <button v-if="saveState === 'saved' && !restarted" class="btn ghost" @click="apply">Apply</button>
        </div>
    ...
    <div v-else class="text-xs opacity-60 space-y-2">
      ...
      <div v-else-if="isError" class="text-red-600 dark:text-red-400 space-y-2">
        ...
        <button class="btn" @click="store.reloadConfig?.()" :disabled="isLoading">
          Retry
        </button>
      </div>
...
watch(() => route.query.sec, (id) => typeof id === 'string' && scrollToOpen(id));
...
function buildSearchIndex() {
  ...
  for (const sec of Array.from(root.querySelectorAll('section[id]'))) {
    ...
    for (const lbl of Array.from(sec.querySelectorAll('label'))) {
      ...
      let target = null;
      if (forId) {
        try { target = sec.querySelector('#' + CSS.escape(forId)); } catch { }
      }
...
watch(searchQuery, (q) => {
  ...
  searchResults.value = v
    ? searchIndex.value.filter((it) => it.label.toLowerCase().includes(v) || it.path.toLowerCase().includes(v)).slice(0, 15)
    : [];
});
...
.flash-highlight {
  box-shadow: 0 0 0 3px rgba(253, 184, 19, .35);
  outline: 2px solid rgba(253, 184, 19, .35);
  border-radius: 6px;
