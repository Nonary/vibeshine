...
function readThemePreference(): ThemePreference {
  const value = localStorage.getItem('vibeshine.theme');
...
export const useSystemStore = defineStore('system', () => {
  ...
  const error = ref('');
  const navCollapsed = ref(storedBoolean('vibeshine.nav-collapsed', false));
  ...
  function setTheme(value: ThemePreference): void {
    theme.value = value;
    localStorage.setItem('vibeshine.theme', value);
    applyTheme();
  }

  function toggleNav(): void {
    navCollapsed.value = !navCollapsed.value;
    localStorage.setItem('vibeshine.nav-collapsed', String(navCollapsed.value));
  }
