  ...
  fs::path appdata() {
    ...
    std::call_once(migration_flag, []() {
      ...
      if ((dir = getenv("CONFIGURATION_DIRECTORY")) != nullptr && strlen(dir) > 0) {
        found = true;
        config_path = fs::path(dir) / "vibeshine"sv;
      }
      ...
      if (!found) {
        ...
        config_path = fs::path(homedir) / ".config/vibeshine"sv;
      }

      // migrate from the old config location if necessary
      migrate_envvar = getenv("VIBESHINE_MIGRATE_CONFIG");
      if (migrate_config && found && migrate_envvar && strcmp(migrate_envvar, "1") == 0) {
        std::error_code ec;
        fs::path old_config_path = fs::path(homedir) / ".config/vibeshine"sv;
