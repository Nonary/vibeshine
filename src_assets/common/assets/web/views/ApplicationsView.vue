    ...
    <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">
      ...
      >
        ...
        <div class="p-3 flex flex-col flex-1 min-h-[110px]">
          ...
          <p class="text-[10px] uppercase tracking-wide opacity-60 mb-2 line-clamp-2">
            {{ displayCmd(app) }}
          </p>
          <div class="mt-auto flex items-center justify-between text-[10px]">
            <span
              class="px-2 py-0.5 rounded-full bg-green-500/15 text-green-600 dark:text-green-400"
              >Configured</span
            >
...
async function reload() {
  ...
  if (!platform.value) {
    try {
      platform.value = await fetch('./api/config')
        .then((r) => r.json())
        .then((r) => r.platform);
    } catch {}
  }
