...
if (-not $releaseExists) {
  ...
  if ($NotesPath -and (Test-Path -LiteralPath $NotesPath)) {
    ...
    $createArgs += @("--notes", "Pinned Windows x64 WebRTC artifacts for Vibeshine.")
  }
  if ($env:GITHUB_SHA) {
...
if ($existingAsset.Count -gt 0) {
  $tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) "vibeshine-webrtc-publish-$([System.Guid]::NewGuid().ToString('N'))"
