#ifndef VIBESHINE_DISPLAY_POWER_H
#define VIBESHINE_DISPLAY_POWER_H
...
static gboolean display_power_refresh(gpointer data) {
  ...
  if (!ready && !state->retrying) {
    fprintf(stderr, "vibeshine-session-broker: retrying display power recovery\n");
  } else if (ready && state->retrying) {
    fprintf(stderr, "vibeshine-session-broker: display power recovered\n");
  }
...
static int run_display_power(bool require_inhibit) {
  ...
  if (!state.bus) {
    fprintf(stderr, "vibeshine-session-broker: display power bus: %s\n", error->message);
    g_clear_error(&error);
    return 126;
  }
  ...
  alarm(0);
  if (state.failed) fprintf(stderr, "vibeshine-session-broker: display wake/inhibition failed\n");
